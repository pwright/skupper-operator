...
NOTE: The Skupper Operator does not require that you create a CRD.

. Create a YAML file defining the ConfigMap of the site you want to create, for example:
+
[source,bash,options="nowrap"]
----
apiVersion: v1
...
----
+
Edit the `console-password` value before proceeding.
...
. Verify that the site is created by checking that the Skupper router, service controller and site controller pods are running:
+
[source,bash,options="nowrap"]
----
$ oc get pods
